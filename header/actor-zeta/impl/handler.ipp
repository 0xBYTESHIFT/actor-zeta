...
namespace actor_zeta { namespace base {
    ...
    using forward_arg =
        typename std::conditional<std::is_lvalue_reference<type_traits::type_list_at_t<List, I>>::value,
                                  typename std::add_lvalue_reference<std::decay_t<type_traits::type_list_at_t<List, I>>>::type,
                                  typename std::add_rvalue_reference<std::decay_t<type_traits::type_list_at_t<List, I>>>::type>::type;
    ...
    struct transformer {
        auto operator()(F&& f) -> std::function<void(communication_module&)> {
            return [f](communication_module& ctx) -> void {
                ...
                constexpr int args_size = call_trait::number_of_arguments;
                apply_impl(f, ctx, std::make_index_sequence<args_size - 1>{});
            };
        }
    };
    ...
    struct transformer<F, Args, 2> final {
        auto operator()(F&& f) -> std::function<void(communication_module&)> {
            return [f](communication_module& ctx) -> void {
                ...
                using clear_args_type_2 = std::decay_t<arg_type_2>;
